...
async def get_pipeline_by_stage(session: AsyncSession) -> list[dict]:
    q = (
...
async def get_dashboard(session: AsyncSession, user_id: UUID | None = None) -> dict:
    cache_key = dashboard_cache_key(user_id) if user_id else "dashboard:anon"
    cached = await cache_get_json(cache_key)
    if cached:
        return cached
    summary = await get_summary_stats(session)
    pipeline = await get_pipeline_by_stage(session)
